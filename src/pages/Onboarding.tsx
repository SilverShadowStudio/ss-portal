        ...
        <div className="absolute left-0 right-0 top-full z-50 max-h-52 overflow-y-auto border border-border bg-background">
          {filtered.map((country) => (
            <button
              ...
              className={`w-full text-left px-4 py-2 font-sans transition-colors hover:bg-muted/30 ${
                country === value ? "text-gold" : "text-foreground/70 hover:text-foreground"
              }`}
              style={{ fontSize: 12 }}
            >
              {country}
      ...
      {lines.map((line, i) => {
        ...
        return (
          <p
            key={i}
            style={{
              marginBottom: isItem || isDef ? "10px" : "20px",
              paddingLeft:  isItem || isDef ? "28px" : 0,
              textIndent:   isItem || isDef ? "-28px" : 0,
            }}
          >
            {line}
          </p>
...
const NDA_SECTIONS: Array<{ title: string; body: string[] }> = [
  {
    title: "Background",
    body: [
      `(A) The Parties wish to explore, negotiate and potentially perform a business engagement relating to CGI production services and related deliverables provided by the Counterparty to Silvershadow Studio Limited (the "Purpose").`,
      `(B) In connection with the Purpose, each Party may disclose Confidential Information to the other. This Agreement sets out the terms on which such Confidential Information will be protected.`,
    ],
  },
  {
    title: "1. Definitions",
  ...
  },
  {
    ...
    body: [
      ...
      `(c) protect the Confidential Information using at least the same standard of care it applies to its own confidential information of similar importance, and in any event no less than a reasonable standard of care;`,
      `(d) not copy, reproduce or reduce to writing any part of the Confidential Information except as reasonably necessary for the Purpose;`,
      `(e) disclose the Confidential Information only to Permitted Recipients; and`,
      `(f) remain liable for any breach of this Agreement by its Permitted Recipients as if such breach were its own.`,
  ...
  },
  {
    ...
    body: [
      ...
      `(a) was lawfully in its possession before disclosure, free of any obligation of confidence;`,
      `(b) is or becomes publicly available through no act or omission of the Receiving Party or its Permitted Recipients;`,
      `(c) is lawfully obtained from a third party who is not under any obligation of confidence in respect of it; or`,
  ...
  {
    title: "5. Marketing and Portfolio Rights",
    body: [
  ...
  },
  {
    ...
    body: [
      ...
      `10.2  Notices shall be deemed received:`,
      `(a) if delivered by hand, on delivery;`,
      `(b) if posted within the UK, on the second working day after posting, and if posted internationally, on the tenth working day;`,
      `(c) if sent by email, on the next working day after sending.`,
    ],
  },
  {
    title: "11. General",
...
// ── Sign button + back button (shared style) ───────────────────────────────────
...
function NdaPage({ form, today, onNext, onBack }: {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      >
        <header>
          <p className="uppercase text-foreground/50" style={{ fontSize: "10px", letterSpacing: "0.3em", marginBottom: "40px" }}>
            MNDA-1.0
          </p>
          <h1 className="font-serif font-normal text-foreground/90" style={{ fontSize: "46px", letterSpacing: "-0.005em", lineHeight: 1.05 }}>
            Mutual Non-Disclosure Agreement
          </h1>
          <p className="uppercase text-foreground/45" style={{ fontSize: "10px", letterSpacing: "0.3em", marginTop: "40px" }}>
            {dateStr}
          </p>
        </header>

        <section style={{ marginTop: "72px" }}>
          <p className="uppercase text-foreground/45" style={{ fontSize: "10px", letterSpacing: "0.3em", marginBottom: "22px" }}>
            Parties
          </p>
          ...
          <div ref={checkboxRef} style={{ marginTop: "64px" }}>
            <label className="flex items-start gap-5 cursor-pointer">
              ...
              <span className="text-foreground/85" style={{ fontSize: "15px", lineHeight: 1.75 }}>
                I have read and agree to the Mutual Non-Disclosure Agreement above.
              </span>
            </label>
            {showError && !accepted && (
              <p className="text-foreground/55 uppercase" style={{ fontSize: "10px", letterSpacing: "0.22em", marginTop: "18px", marginLeft: "34px" }}>
                Acceptance is required to proceed.
              </p>
            )}
          </div>
          <div style={{ marginTop: "56px" }}>
            <button onClick={handleSign} className={SIGN_BTN} style={SIGN_BTN_STYLE}>
              Sign NDA
            </button>
          </div>
          <div style={{ marginTop: "64px" }}>
            <button onClick={onBack} className={BACK_BTN} style={BACK_BTN_STYLE}>
              Back to details
            </button>
          </div>
        </div>
...
function FsaPage({ form, today, onBack, signing, onSign }: {
  form: FormData; today: Date; onBack: () => void; signing: boolean; onSign: () => void;
}) {
  const [accepted, setAccepted]   = useState(false);
  const [showError, setShowError] = useState(false);
  const checkboxRef = useRef<HTMLDivElement>(null);
        ...
        <section style={{ marginTop: "72px" }}>
          ...
          <div className="text-foreground/85" style={{ fontSize: "15px", lineHeight: 1.9 }}>
            <p style={{ marginBottom: "20px" }}>
              This Freelance Services Agreement is made on {dateStr} by and between:
            </p>
            <p style={{ marginBottom: "8px" }}>
              <span className="text-foreground/55">Client:</span> Silvershadow Studio Limited, 332 Ladbroke Grove, London, W10 5AD. Company No: 9178937. VAT Number: GB 232 8467 02. ("Client")
            </p>
            <p style={{ marginBottom: "8px" }} className="text-foreground/45">and</p>
            <p>
              <span className="text-foreground/55">Contractor:</span> {fullName}, {address}. ("Contractor")
            </p>
          </div>
        </section>
        ...
        <div style={{ marginTop: "128px" }}>
          ...
          <div style={{ marginTop: "56px" }}>
            <button onClick={handleSign} disabled={signing} className={SIGN_BTN} style={SIGN_BTN_STYLE}>
              {signing ? (
                <><Loader2 className="h-3 w-3 animate-spin mr-2" />Signing…</>
              ) : "Sign Agreement"}
            </button>
...
export default function Onboarding() {
  ...
  const inputClass = (field: keyof FormData) =>
    `w-full border-0 border-b bg-transparent py-3 text-foreground focus:outline-none transition-colors text-sm ${borderFor(field)}`;

  const selectClass = (field: keyof FormData) =>
    `w-full border-0 border-b bg-transparent py-3 text-foreground focus:outline-none transition-colors appearance-none cursor-pointer text-sm ${borderFor(field)}`;
    ...
    <div className="min-h-screen bg-background px-4 py-16">
      <div className="mx-auto max-w-2xl">
        ...
        <div className="space-y-14 animate-fade-in" style={{ animationDelay: "0.1s" }}>
          ...
          <section>
            ...
            <div className="space-y-7">
              ...
              <div className="space-y-1.5" data-field="country">
                <label className="font-sans uppercase text-foreground/40" style={{ fontSize: 9, letterSpacing: "0.2em" }}>
                  {FIELD_LABELS.country} <span className="text-gold">*</span>
                </label>
                <CountryCombobox
                  value={form.country}
          ...
          <button
            onClick={handleNext}
            className="w-full flex items-center justify-center gap-2 bg-foreground text-background py-4 font-sans uppercase hover:bg-foreground/90 transition-colors"
