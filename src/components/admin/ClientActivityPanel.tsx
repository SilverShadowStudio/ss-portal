...
export function ClientActivityPanel() {
  ...
  useEffect(() => {
    (async () => {
      ...
      let q = supabase
        ...
        .select("id, user_id, actor_name, actor_role, kind, session_id, path, started_at, ended_at, duration_ms")
      ...
      const { data } = await q;
      const filtered = ((data ?? []) as Row[]).filter((r) => !adminIds.has(r.user_id));
      setRows(filtered);
      setLoading(false);
    })();
  ...
  const sessions: SessionSummary[] = useMemo(() => {
    ...
    bySid.forEach((items, sid) => {
      ...
      const end = [...items].reverse().find((i) => i.kind === "session_end");
      const startTs = new Date((start ?? sortedAsc[0]).started_at).getTime();
      const endTs = end
        ? new Date(end.ended_at ?? end.started_at).getTime()
        : new Date(
            sortedAsc[sortedAsc.length - 1].ended_at ?? sortedAsc[sortedAsc.length - 1].started_at,
          ).getTime();
      const pageMs = items
  ...
  const pages: PageSummary[] = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      const cur = byPath.get(r.path) ?? { path: r.path, visits: 0, totalMs: 0 };
      cur.visits += 1;
    ...
    <div>
      <div className="mb-6 flex flex-wrap items-end gap-4">
        <div className="space-y-1.5">
          <label className="text-[10px] font-bold tracking-[0.18em] uppercase text-muted-foreground">Client</label>
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] font-bold tracking-[0.18em] uppercase text-muted-foreground">From</label>
          <Input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="w-[160px]" />
        </div>
        <div className="space-y-1.5">
          <label className="text-[10px] font-bold tracking-[0.18em] uppercase text-muted-foreground">To</label>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-border bg-card p-12 text-center">
          ...
          <p className="font-serif text-base text-muted-foreground">No client activity in this range.</p>
        </div>
      ) : (
            ...
            <ul className="divide-y divide-border/40 max-h-[60vh] overflow-y-auto">
              {sessions.map((s) => (
                <li key={s.sessionId} className="py-4 grid grid-cols-3 items-center gap-4">
                  <p className="text-sm font-medium text-foreground truncate">{s.actorName ?? "Unknown"}</p>
                  <div className="text-left">
                    <p className="font-serif text-base text-foreground tracking-wide">
                      {format(new Date(s.start), "HH:mm")}{" — "}
                      {s.end ? format(new Date(s.end), "HH:mm") : <span className="italic text-muted-foreground">ongoing</span>}
                    </p>
              ...
              {sessions.length === 0 && (
                <li className="py-6 text-center text-xs text-muted-foreground">No sessions</li>
              )}
            </ul>
          </div>
          <div className="rounded-xl border border-border bg-card shadow-sm p-6">
            ...
            <ul className="divide-y divide-border/40 max-h-[60vh] overflow-y-auto">
              {pages.map((p) => (
                <li key={p.path} className="py-3 flex items-center gap-4">
                  <code className="text-xs text-foreground truncate flex-1">{p.path}</code>
              ...
              {pages.length === 0 && (
                <li className="py-6 text-center text-xs text-muted-foreground">No page views</li>
              )}
            </ul>
          </div>
        </div>
