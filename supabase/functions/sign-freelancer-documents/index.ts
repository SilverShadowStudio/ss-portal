...
function buildFsaClauses(rateStr: string): Array<{ title: string; body: string }> {
  return [
    ...
    { title: '13. Bank Holidays and Weekends', body: 'The Contractor is not required to work on weekends or during the eight standard UK Bank Holidays: New Year\'s Day, Good Friday, Easter Monday, Early May Bank Holiday, Spring Bank Holiday, Summer Bank Holiday, Christmas Day, Boxing Day.' },
    { title: '14. Entire Agreement', body: 'This Agreement constitutes the entire agreement between the Parties and supersedes all prior oral or written understandings, including any prior non-disclosure or confidentiality agreement between the Parties relating to the same subject matter. Any changes must be made in writing and signed by both Parties.' },
    { title: '15. Governing Law', body: 'This Agreement shall be governed by the laws of England and Wales. Any disputes shall be subject to the exclusive jurisdiction of the courts of England.' },
    { title: '16. Signatures', body: 'IN WITNESS WHEREOF, the parties have executed this Agreement on the date first above written.' },
  ]
}
...
function generateFsaPdf(p: SignPayload, now: Date, backgroundHex: string, contractorSigDataUrl?: string, studioSigDataUrl?: string): Uint8Array {
  ...
  for (const clause of clauses) {
    ...
    if (clause.title !== '16. Signatures') writeBody(clause.body, { color: 70 })
  }
