...
function buildPortfolioUrl(args: {
  projectId?: string | null
  sceneId?: string | null
  roundId?: string | null
}): string {
  const params = new URLSearchParams()
  if (args.projectId) params.set('project', args.projectId)
  if (args.sceneId) params.set('scene', args.sceneId)
  if (args.roundId) params.set('round', args.roundId)
  const qs = params.toString()
  return qs ? `${PORTFOLIO_ORIGIN}/portfolio?${qs}` : `${PORTFOLIO_ORIGIN}/portfolio`
}
...
Deno.serve(async (req) => {
  ...
  const ctaUrl = buildPortfolioUrl({
    projectId: payload.project_id ?? null,
    sceneId: payload.scene_id ?? null,
    roundId: payload.round_id ?? null,
  })
  const html = buildDeliveryEmailHtml({ projectName, sceneName, roundLabel, ctaUrl })
