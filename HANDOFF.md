...
- **Airtable address sync: per-field PATCHes, no auto column creation.** Pre-existing `composeAddress` helper (single-line "21 Knightsbridge, London, SW1X 7LY, UK" string) was deleted. Each portal column maps to its own configured Airtable column. Kieran owns column creation in his base; admin then updates `app_settings.airtable_contact_field_config` keys.
- **`SIGNED_IN` de-dupe via `auth.users.last_sign_in_at` comparison, not via local ref guard.** A ref would protect a single tab; the server-side `last_sign_in_at` covers cross-tab / cross-device duplicates too. 10 s skew tolerance absorbs the few-second gap between auth-side stamp and client-side activity_log insert.
- **`admin-delete-account` deletes the account row first, then auth users.** Order chosen so `account_members` cascades automatically and we don't fight FK constraints. Failures on individual auth deletes are logged + returned in `auth_users_failed`, never block the account-level delete.
- **`session_end` idempotence via `sessionEndedRef` boolean, not by debouncing.** The `pagehide` / `visibilitychange` flushers both call the same beacon; a boolean guard is the smallest correct fix. Reset on new `session_start`.
- **Vercel proxy chosen over Supabase Custom Domains for verify links.** Custom Domains requires Supabase Pro (~$25/month). Vercel proxy is zero-cost, ships in one commit, no DNS work. Caveat: doesn't cover Supabase's own auth emails (password reset etc., still hosted on supabase.co). For invite/agreement/quote/invoice emails which all go through our edge-function + Resend pipeline, the proxy is enough.
- **Portal-hosted images chosen over base64 inlining.** Outlook desktop doesn't render `data:` URIs in `<img src>`. Vercel-static under `/email-assets/` is aligned with sender domain, cached at the edge, free, and Outlook-compatible.
- **Manual invite uses `magiclink`, not `invite`.** Mirrors `admin-create-client`'s resend mode. `invite` 400s for already-registered users; `magiclink` works for any existing user including never-confirmed ones (it confirms them AND signs them in on click).
